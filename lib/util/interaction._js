...
exports.promptPasswordOnce = function (cli, msg, callback) {
  exports.passwordOnce(cli, msg, '*', function (result) {
    callback(null, result);
  });
};
...
exports.chooseIfNotGiven = function (cli, promptString, progressString, currentValue, valueProvider, optionsOrCallback, callback) {
  var options = optionsOrCallback;
  if (__.isFunction(optionsOrCallback)) {
  ...
  if (!__.isUndefined(currentValue)) {
    ...
    valueProvider(function (err, values) {
      ...
      if (options.limit && values.length > options.limit) {
        exports.prompt(cli, promptString, function (err, value) {
          if (err) { return callback(err); }
          return callback(null, value);
        });
      } else {
        cli.output.help(promptString);
        exports.choose(cli, values, function (err, i) {
          if (err) { return callback(err); }
          return callback(null, values[i]);
        });   
      }
    });
  }
