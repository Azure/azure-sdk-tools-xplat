...
Account.prototype._getSubscriptionInCloudConsole = function (callback) {
  var tokens = process.env.AZURE_CONSOLE_TOKEN.split(';');
  self = this; 
  var armTokenEntry = self._convertToken(tokens[0]);
  ...
  var armClient = self._getArmClient(credential);
  var subscriptions = [];
  self._tokenCache.add([armTokenEntry], function (err) {
    if (err) {
      return callback(err);
    }
    armClient.subscriptions.list(function (err, result) {
      ...
      tokenEntries = [armTokenEntry];
      if (tokens[1]) {
        tokenEntries.push(self._convertToken(tokens[1]));
        if (tokens[2]) {
          tokenEntries.push(self._convertToken(tokens[2]));
        }
      }
      self._tokenCache.add(tokenEntries, function (err) {
        if (err) {
          return callback(err);
        }
...
Account.prototype._convertToken = function (accessToken) {
