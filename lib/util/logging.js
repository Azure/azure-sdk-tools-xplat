  ...
  if (options.json) {
    ...
    defaultTransport.formatter = function (options) {
      if (options.meta && Object.keys(options.meta).length || !options.message) {
        ...
        return options.level + ': ' + options.message; //TODO: remove color marks?
      }
    };
    //do not set 'json' flags on transport, as winston makes some assumption we don't want such as the deep clone of the "meta" field
  } else {
    log.padLevels = true;
