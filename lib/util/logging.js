...
log.createLogFilter = function () {
  return function handle(resource, next, callback) {
    ...
    if(resource.body && resource.body.length > 2 * 1024 * 1024) { // if the body is large, log only that the body was too large to log and was more than likely file content
    ...
    return next(resource, function (err, response, body) {
      ...
      if (response) {
        var bodyToLog = body;
        if(body && body.length > 2 * 1024 * 1024) { // if the body is large, log only that the body was too large to log and was more than likely file content
