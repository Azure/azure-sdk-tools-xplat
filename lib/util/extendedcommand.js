...
_.extend(ExtendedCommand.prototype, {
  ...
  helpCategories: function(levels, depth) {
    ...
    for (var name in this.categories) {
      ...
      if (levels === -1 || levels > 0) {
        if (depth < 1) {
          ...
          var methods = [];
          for (var index2 in cat.commands) {
            var cmd2 = cat.commands[index2];
            methods.push(cmd2.name);
          }
          this.output.help(' ', cat.fullName() + ' (' + methods.join('|') + ') [options]');
        }

        cat.helpCategories(levels !== -1 ? --levels : -1, depth + 1);
      } else {
