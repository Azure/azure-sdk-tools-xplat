...
function operateOnPolicy(policySettings, operation, _) {
  ...
  try {
    ...
    if (policySettings.accessType === StorageUtil.AccessType.Container) {
      StorageUtil.performStorageOperation(policySettings.setAclOperation, _, policySettings.resourceName, policies, null, policySettings.storageOptions);
    } else if (policySettings.accessType === StorageUtil.AccessType.Queue) {
    ...
    } else if (policySettings.accessType === StorageUtil.AccessType.Table) {
      policySettings.storageOptions.signedIdentifiers = policies;
      StorageUtil.performStorageOperation(policySettings.setAclOperation, _, policySettings.resourceName, policySettings.storageOptions);
    }
  } catch (e) {
    throw new Error((util.format($('%s\rReason: %s'), e.message, e.reason)));
  } finally {
