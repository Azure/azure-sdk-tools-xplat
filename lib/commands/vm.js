...
var CommunityUtil = require('../communityutil');
    ...
    endpointsAsList.forEach(function(endpointInfoStr, j) {
      ...
      endpointInfoAsList.forEach(function(item) {
        ...
        if (keys[i] === 'lb_port' || keys[i] === 'vm_port') {
          ...
          probeportAsInt = parseInt(item, 10);
  ...
  function createVM(options, cmdCallback) {
    ...
    function cmdCallbackHook(error) {
      if (communityImgName) {
        ...
        imageDelete(communityImgName, imageDeleteOptions, function(error1) {
          cmdCallback(error);
        });
      } else {
        cmdCallback(error);
      ...
      if (options.community) {
        ...
        communityUtil.resolveUid(options.imageName, function(error, response) {
          ...
          if (!error) {
            ...
            cmdCallback('Failed to validate Community image');
          }
        });
      } else {
        doCreate();
          ...
          if (!error) {
            ...
            progress = cli.progress('Updating network configuration');
              utils.doServiceManagementOperation(channel, 'modifyRole', found.svc, found.deploy.Name,
                options.name, vmRole, function(error, response) {
                progress.end();
                var errorObj = (error == null) ? null : new Error(error);
                return cmdCallback(errorObj);
              });
            } else {
               progress.end();
