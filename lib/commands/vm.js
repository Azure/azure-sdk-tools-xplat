...
exports.init = function(cli) {
  ...
  vm.command('create <dns-name> <image> <user-name> [password]')
    ...
    .option('-P, --no-ssh-password', 'remove password support when using --ssh-cert')
    ...
    .execute(function(dnsName, image, userName, password, options, callback) {
      ...
      if (options.ssh) {
        ...
        callback('--no-ssh-password can only be used with the --ssh-cert parameter');
      }
  ...
  vm.command('availset <vm-name> <availability-set>')
    .usage('<vm-name> <availability-set>')
    .description('Assign an Azure VM to an availability set. If not seen before creates new availability set.')
    .option('-s, --subscription <id>', 'use the subscription id')
    .option('-d, --dns-name <name>', 'only show VMs for this DNS name')
    .execute(function(vmName, availabilitySet, options, callback) {
      assignAvailabilitySet(vmName, availabilitySet, options, callback);
    });
  ...
  .execute(function(vmName, endpoints, options, callback) {
    ...
    endpointsAsList.forEach(function(endpointInfoStr, j) {
      ...
      if (endpointInfoAsList.length > 6) {
        return callback(new Error(message));
      }
      ...
      endpointInfoAsList.forEach(function(item) {
        ...
        if (keys[i] === 'lb_port' || keys[i] === 'vm_port') {
          ...
          if ((item != portAsInt) || (portAsInt > 65535)) {
            return callback(new Error('lb-port or vm-port must be an integer less than or equal to 65535 (endpoint ' + (j + 1) + ')'));
          }

          if (keys[i] === 'lb_port')
          {
            inputEndPoint.Name = 'endpname-' + item + '-';
          ...
          if (endpointInfoAsList.length < 5) {
            return callback(new Error('probe-port is required to enable probing (endpoint ' + (j + 1) + ')'));
          }
          ...
          if (item === 'tcp' || item == 'http') {
            ...
            return callback(new Error('Possible values for probe-protocol are tcp and http (endpoint ' + (j + 1) + ')'));
          }
        } else if (keys[i] === 'prob_port') {
          var probeportAsInt = parseInt(item, 10);
          if ((item != probeportAsInt) || (probeportAsInt > 65535)) {
            return callback(new Error('probe-port must be an integer less than or equal to 65535 (endpoint ' + (j + 1) + ')'));
          }

          inputEndPoint.LoadBalancerProbe.Port = probeportAsInt;
          if (endpointInfoAsList.length == 5 && inputEndPoint.LoadBalancerProbe.Protocol == 'http') {
          ...
          if (inputEndPoint.LoadBalancerProbe.Protocol == 'http') {
            ...
            logger.warn('probe-path option will be ignored when probe-protocol is tcp (endpoint ' + (j + 1) + ')');
          }
        }
  ...
  endpoint.command('show <vm-name>')
    .usage('<vm-name>')
    .description('Show details of endpoints of an azure VM')
    .option('-s, --subscription <id>', 'use the subscription id')
    .option('-d, --dns-name <name>', 'only show VMs for this DNS name')
    .execute(function(name, options, callback) {
  ...
  function createVM(options, cmdCallback) {
    ...
    function doCreate() {
      ...
      utils.doServiceManagementOperation(channel, 'listOSImage', function(error, response) {
        ...
        if (!error) {
          ...
          if (!image) {
            cmdCallbackHook('Image \'' + options.imageName + '\' not found');
          }
          logger.silly('image:');
    ...
    function loadSshCert() {
      ...
      if (!utils.isPemCert(pemSshCertStr)) {
        cmdCallbackHook('Specified SSH certificate is not in PEM format');
      }
          ...
          function(error, imageTargetUrl) {
            if (error) {
              ...
              if (imageTargetUrl.indexOf('$root') >= 0) {
                cleanupHostedServiceAndExit('Creating OS disks in $root storage container is not supported. Storage URL: ' + imageTargetUrl);
              }
          ...
          utils.doServiceManagementOperation(channel, 'getDeploymentBySlot', dnsPrefix, 'Production', function(error, response) {
            ...
            if (error) {
              ...
              if (!options.connect) {
                ...
                hookEx = true;
                cmdCallbackHook('A VM with dns prefix \'' + dnsPrefix + '\' already exists');
              }
      ...
      utils.doServiceManagementOperation(channel, 'listHostedServices', function(error, response) {
        ...
        if (error) {
          ...
          if (service) {
            ...
            if (options.location && options.affinityGroup) {
              cmdCallbackHook('both --location and --affinitygroup options are specified');
            }
    ...
    enumDeployments(channel, options, function() {
      ...
      if (options.rsps.length === 0 && options.errs.length > 0) {
        ...
        if (options.errs[0].code !== 'ResourceNotFound' ||
          ...
          return cmdCallback(options.errs[0]);
        }
      }
  ...
  function assignAvailabilitySet(name, availabilitySet, options, cmdCallback) {
    var channel = utils.createServiceManagementService(cli.category('account').lookupSubscriptionId(options.subscription),
      cli.category('account'), logger);

    logger.info("Service name: " + name);
    logger.info("Availability set: " + availabilitySet);
    ...
    enumDeployments(channel, options, function() {
      ...
      if (options.rsps.length === 0 && options.errs.length > 0) {
        ...
        for (var i = 0; i < options.rsps.length; i++) {
          ...
          if (roles) {
            for (var j = 0; j < roles.length; j++) {
              logger.info("Looking at role " + j + "(" + i + ") of type " + roles[j].RoleType + " and name = " + roles[j].RoleName);
              if (roles[j].RoleType === 'PersistentVMRole' &&
                  roles[j].RoleName === name) {
                if (found) {
                  // found duplicates
        ...
        if (found) {
          ...
          utils.doServiceManagementOperation(channel, 'getRole', found.svc, found.deploy.Name, name, function(error, response) {
            if (!error) {
              var role = response.body;
              var configurationSets = role.ConfigurationSets;
              progress.end();
              logger.info("Creating vmRole, availabilitySet = " + availabilitySet);
              var vmRole = {
                ConfigurationSets: configurationSets,
                AvailabilitySetName: availabilitySet
              };

              logger.info("About to call modifyRole, vmRole.AvailabilitySetName = " + vmRole.AvailabilitySetName);
              progress = cli.progress('Updating availability set...');
              utils.doServiceManagementOperation(channel, 'modifyRole', found.svc, found.deploy.Name,
                  name, vmRole, function(error) {
                progress.end();
                cmdCallback(error);
              });
            } else {
  ...
  function deleteRoleOrDeployment(channel, svcname, deployment, vmname, callback) {
    ...
    if (deployment.RoleList.length > 1) {
      utils.doServiceManagementOperation(channel, 'deleteRole', svcname, deployment.Name, vmname, function(error) {
        if (!error) {
          callback();
        } else {
          callback(error);
    ...
    utils.doServiceManagementOperation(channel, 'getHostedService', dnsPrefix, function(error, response) {
      if (!error) {
        if (response.body.HostedServiceProperties.Description === 'Implicitly created hosted service') {
          ...
          enumDeployments(channel, options, function() {
            if (options.rsps.length === 0) {
              utils.doServiceManagementOperation(channel, 'deleteHostedService', options.dnsPrefix, function(error) {
                if (!error) {
                  callback();
                } else {
                  callback(error);
  ...
  function createPrettyVMView(role, deployment) {
    ...
    return {
      ...
      InstanceUpdateDomain: roleInstance.InstanceUpgradeDomain,
