...
exports.init = function (cli) {
  ...
  function promptIfNotGiven(prompt, value, _) {
    var result = interaction.promptIfNotGiven(cli, prompt, value, _);
    if(result.length === 0) {
  ...
  */
  ...
  mobile.enablePreview = function (options, feature, callback) {
    ...
    channel.send('POST', feature, callback);
  };
  ...
  mobile.loadAllScripts = function(options, callback) {
    ...
    try {
      ...
      mobile.getCustomApis(options, createCallback('api'));
    } catch (e) {
      // Do nothing, shared scripts purposely throws an error
    }
  ...
  mobile.command('create [servicename] [username] [password]')
        ...
        .execute(function (servicename, username, password, options, callback) {
          if (options.sqlDb && !options.sqlServer) {
            return callback('To use an existing SQL database, you must specify the name of an existing SQL server using the --sqlServer parameter.');
          }
          ...
          function ensuredLocation(location) {
            ...
            function ensuredServiceName(servicename) {
              ...
              if (servicename.length < 2 || servicename.length > 48) {
                return callback('Service name must be between 2 and 48 characters.');
              } else if(!servicename.match(/^[a-zA-Z][0-9a-zA-Z-]*[0-9a-zA-Z]$/)) {
                return callback('Service name must start with a letter, contain only letters, numbers, and hyphens, and end with a letter or number.');
              }
              ...
              function ensuredUsername(username) {
                ...
                function promptPassword() {
                  interaction.password(cli, 'SQL administrator password: ', '*', function (password) {
              ...
              function doProceed(decision) {
                ...
                function doStep(stepIndex) {
                  if (stepIndex == plan.length) {
                    return callback(failures > 0 ? 'Not all delete operations completed successfully.' : null);
                  }
  ...
  mobile.command('log [servicename]')
        ...
        .execute(function (servicename, options, _) {
          ...
          interaction.formatOutput(cli, result, function(logs) {
          ...
          function ensuredUnhealthyServiceName(unhealthyservice) {
            ...
            function ensuredHealthyServiceName(healthyservice) {
              ...
              if(options.quiet) {
                ...
                cli.confirm('Warning: this action will use the capacity from the mobile service \'' + healthyservice + '\' and delete it. Do you want to recover the mobile service \'' + unhealthyservice + '\'? (y/n): ', doProceed);
              }
  ...
  mobileConfig.command('list [servicename]')
        ...
        .execute(function (servicename, options, _) {
          ...
          interaction.formatOutput(cli, results, displayConfiguration);
  ...
  mobileConfig.command('get [servicename] [key]')
        ...
        .execute(function (servicename, key, options, _) {
          ...
          if (!getConfigHandlers[key]) {
            ...
            throw new Error('Unsupported key ' + key.red);
          }

          var result;
          var progress = cli.progress('Retrieving setting: ' + key);
          try {
          ...
          interaction.formatOutput(cli, value, function (output) {
            if (output[key]) {
    ...
    if (typeof permissions == 'string') {
      permissions.split(',').forEach(function (pair) {
        ...
        if (match[1] !== '*' && !keys.some(function (key) { return key === match[1]; })) {
          throw new Error('Unsupported key \'' + match[1].red + '\', must be one of *, ' + keys.join(', '));
        }

        if (!roles.some(function (role) { return role === match[2]; })) {
          throw new Error('Unsupported permission value "\'' + match[2].red + '\'". Permission must be one of ' + roles.join(', '));
        }
              ...
              function doProceed(decision) {
                if (!decision) {
                  ...
                  var doStep = function (stepIndex) {
                    if (stepIndex == plan.length) {
                      return callback(failures > 0 ? 'Not all update operations completed successfully.' : null);
                    }
  ...
  function checkScriptName(options) {
    if (!options.script) {
      ...
      log.info('For the APNS scripts, specify shared/apnsFeedback');
      log.info('For scheduler scripts, specify scheduler/<scriptName>');
      log.info('For custom API scripts, specify api/<scriptName>');

      throw new Error('Invalid script name ' + options.scriptname);
    }
  }

  function parseScriptName(scriptname) {
    var result = null;
    var match = scriptname.match(/^(table|scheduler|shared|api)\/([^\.]+)/);
    if(!match) {
      return result;
    }
  ...
  function saveScriptFile(scriptSpec, script, output, force) {
    ...
    if (utils.pathExistsSync(file) && !force) {
      throw new Error('File ' + file + ' already exists. Use --override to override.');
    } else {
      try {
        if (!output) {
        ...
        throw new Error('Unable to save file ' + file);
      }
    }
  ...
  mobileScript.command('download [servicename] [scriptname]')
        .usage('[servicename] [scriptname] [options]')
        ...
        .execute(function (servicename, scriptname, options, _) {
          ...
          var progress = cli.progress('Downloading script: \'' + options.scriptname + '\'');
          var script;
          try {
  ...
  mobileScript.command('upload [servicename] [scriptname]')
        ...
        .execute(function (servicename, scriptname, options, _) {
          ...
          try {
            ...
            throw new Error('Unable to read script from file ' + options.file);
          }

          var progress = cli.progress('Uploading script: \'' + options.scriptname + '\'');
          try {
  ...
  mobileScript.command('delete [servicename] [scriptname]')
        ...
        .execute(function (servicename, scriptname, options, _) {
          ...
          var progress = cli.progress('Deleting script: \'' + options.scriptname + '\'');
          try {
        ...
        .execute(function (servicename, options, callback) {
          ...
          if(options.tier !== undefined) {
            if(options.tier !== 'free' && options.tier !== 'standard' && options.tier !== 'premium') {
              return callback('Allowed values for tier are free, standard or premium.');
            }
            ...
            mobile.getScaleSettings(options, function (error, scalesettings) {
              ...
              if(scaleMaxInstanceCounts[newScaleSettings.tier] < newScaleSettings.numberOfInstances) {
                ...
                if(scaleMaxInstanceCounts[newScaleSettings.tier] === 1) {
                  return callback('Number of instances must be set to 1 when the mobile service is in the ' +
                    newScaleSettings.tier + ' tier.');
                }
  ...
  mobileJob.command('update [servicename] [jobname]')
        .usage('[servicename] [jobname] [options]')
        ...
        .execute(function (servicename, jobname, options, _) {
          ...
          if (typeof options.intervalUnits !== 'undefined' && !intervalUnits.some(function (unit) { return unit === options.intervalUnit; })) {
            throw new Error('The --intervalUnit must be one of ' + intervalUnits.join(', '));
          }
          ...
          options.servicename = promptServiceNameIfNotGiven(options, servicename, _);
          options.jobname = interaction.chooseIfNotGiven(cli, 'Scheduled job name: ', 'Retrieving choices', jobname,
            function(cb) {
  ...
  mobilePreview.command('list [servicename]')
        ...
        .execute(function (servicename, options, callback) {
          ...
          function ensuredServiceName(servicename) {
            ...
            mobile.getPreviews(options, function (error, result) {
              if (error) {
                return callback(error);
              }
              ...
              if (log.format().json) {
                ...
                if (result && result.enabled && result.enabled.length > 0) {
                  log.data('Enabled preview features');
                  log.table(result.enabled, function (row, f) {
                      row.cell('Name', f.green);
                    });
                } else {
                  log.data('There are no preview features enabled.');
                }
                log.info('');
                if (result && result.available && result.available.length > 0) {
                  log.data('Available preview features');
                  log.table(result.available, function (row, f) {
                      row.cell('Name', f.green);
                    });
                } else {
  ...
  mobilePreview.command('enable [servicename] [featurename]')
        ...
        .option('-s, --subscription <id>', 'use the subscription id')
        .execute(function (servicename, featurename, options, callback) {
          if (servicename) {
            ensuredServiceName(servicename);
          } else {
            promptString('Mobile service name: ', ensuredServiceName);
          ...
          function ensuredServiceName(servicename) {
            ...
            if (featurename) {
              ensuredFeatureName(featurename);
            } else {
              promptString('Preview feature name: ', ensuredFeatureName);
            }

            function ensuredFeatureName(featurename) {
              options.featurename = featurename;

              var progress = cli.progress('Enabling preview feature for mobile service');
              mobile.enablePreview(options, JSON.stringify({ enable: options.featurename }), function (error, result) {
                  progress.end();
                  if (error) {
                    return callback(error);
                  }
                  ...
                  if (log.format().json) {
                    ...
                    for (var i in result.data)
                    {
                      if (typeof result.data[i] === 'object') {
                        log.data('data.' + i, JSON.stringify(result.data[i]).green);
                      }
                      else
                      {
                        log.data('data.' + i, result.data[i].green);
                      }
                    }
                    log.verbose('Detailed information: ' + result.details);
                  }
  ...
  mobileApi.command('list [servicename]')
        ...
        .execute(function (servicename, options, _) {
          ...
          var progress = cli.progress('Retrieving list of APIs');
          var result;
          try {
          ...
          interaction.formatOutput(cli, result, function(apis) {
  ...
  function displayCustomApi(row, s) {
    ...
    row.cell('Get', s.get);
    row.cell('Put', s.put);
    row.cell('Post', s.post);
    row.cell('Patch', s.patch);
    row.cell('Delete', s.delete);
  }

  mobileApi.command('create [servicename] [apiname]')
        .usage('[servicename] [apiname] [options]')
        .description('Create a Mobile Service custom API')
        .option('-s, --subscription <id>', 'use the subscription id')
        ...
        .execute(function (servicename, apiname, options, _) {
          ...
          // Now create the API
          var progress = cli.progress('Creating custom API: \'' + options.apiname + '\'');
          try {
  ...
  mobileApi.command('update [servicename] [apiname]')
        .usage('[servicename] [apiname] [options]')
        .description('Update a Mobile Service custom API')
        .option('-s, --subscription <id>', 'use the subscription id')
        .option('-p, --permissions <permissions>', 'comma delimited list of <method>=<permission> pairs')
        ...
        .execute(function (servicename, apiname, options, _) {
          ...
          var progress = cli.progress('Updating API: \'' + options.apiname + '\'');
  ...
  mobileApi.command('delete [servicename] [apiname]')
        .usage('[servicename] [apiname] [options]')
        .description('Delete a Mobile Service custom API')
        .option('-s, --subscription <id>', 'use the subscription id')
        .execute(function (servicename, apiname, options, _) {
          options.servicename = promptServiceNameIfNotGiven(options, servicename, _);
          options.apiname = promptIfNotGiven('API name: ', apiname, _);

          var progress = cli.progress('Deleting API: \'' + options.apiname + '\'');
          try {
