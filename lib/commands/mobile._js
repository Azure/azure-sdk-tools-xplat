...
exports.init = function (cli) {
  ...
  function promptTableNameIfNotGiven (options, servicename, _) {
    var result = cli.interaction.chooseIfNotGiven($('Table: '), $('Retrieving choices'), servicename,
      function(cb) {
        mobile.listTables(options, function (error, services) {
          if(error) { cb(error); }
          cb(null, services.map(function(service) { return service.name; }));
        });
      }, _);
  ...
  */
  ...
  mobile.deleteSqlServer = function (options, resource, callback) {
    var sqlService = utils._createSqlClient(cli.category('account').getCurrentSubscription(options.subscription), log);
  ...
  mobileTable.command('update [servicename] [tablename]')
        ...
        .execute(function (servicename, tablename, options, _) {
          ...
          if (failures !== 0) {
            throw new Error($('Not all update operations completed successfuly'));
          }
        });
  ...
  mobileData.command('truncate [servicename] [tablename]')
        ...
        .execute(function (servicename, tablename, options, _) {
          ...
          if(!options.quiet) {
            ...
            if(!cli.interaction.confirm($('Do you really want to delete all data from the table? [y/n]: '))) {
