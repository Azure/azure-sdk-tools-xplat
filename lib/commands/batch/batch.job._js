  ...
  */
  function createJob(jsonFile, options, _) {
    ...
    if (options.template) {
      ...
      userAgent = batchUtil.PREVIEW_USER_AGENT_PREFIX;
    ...
    if (!__.isUndefined(parsedJson.applicationTemplateInfo)) {
      logger.warn('You are using an experimental feature {Application Templates}.');
    ...
    if (parsedJson.taskFactory) {
      ...
      userAgent = batchUtil.PREVIEW_USER_AGENT_PREFIX;
      taskCollection = templateUtils.parseTaskFactory(parsedJson, _);
    ...
    if (shouldGetPool) {
      userAgent = batchUtil.PREVIEW_USER_AGENT_PREFIX;
      pool = jobUtils.getTargetPool(parsedJson, options, _);
      poolOSFlavor = poolUtils.getPoolTargetOSType(pool);
    }
    ...
    if (hasDockerOptions) {
      ...
      batchShipyardUtils.createJobAndAddTasks(parsedJson, taskCollection, pool, options, cli, _);

      return;
    }

    // Send the server request
    var client = batchUtil.createBatchServiceClient(options, userAgent);
