  ...
  */
  function createJob(jsonFile, options, _) {
    ...
    if (parsedJson.poolInfo && parsedJson.poolInfo.autoPoolSpecification && parsedJson.poolInfo.autoPoolSpecification.pool && parsedJson.poolInfo.autoPoolSpecification.pool.packageReferences) {
      ...
      parsedJson.poolInfo.autoPoolSpecification.pool.startTask = templateUtils.constructSetupTask(parsedJson.poolInfo.autoPoolSpecification.pool.startTask, cmds);
    }
    
    var commands = [];
    // Handle package management on tasks
    commands.push(templateUtils.parseTaskPackageReferences(parsedJson, taskCollection));
    ...
    commands.push(templateUtils.processJobForOutputFiles(parsedJson, taskCollection));
    parsedJson.jobPreparationTask = templateUtils.constructSetupTask(parsedJson.jobPreparationTask, commands);
