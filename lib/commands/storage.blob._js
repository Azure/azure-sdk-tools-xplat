...
exports.init = function(cli) {
  ...
  */
  ...
  blob.command('upload [file] [container] [blob]')
    ...
    .option('--concurrenttaskcount <concurrenttaskcount>', 'The concurrent upload threads number.')
  ...
  */
  function getBlobServiceClient(options, _) {
    ...
    if(isConnectionStringDefined && isAccountDefined) {
      ...
      if (isConnectionStringDefined) {
        ...
        var serviceManagementClient = createServiceManagementClient(options.subscription);

        if (!isAccountDefined &&
            (!process.env[StorageUtil.ENV_SDK_ACCOUNT_NAME] ||
             !process.env[StorageUtil.ENV_SDK_ACCOUNT_KEY])) {
          // prompt for storage account name from the current subscription
          options.accountName = interaction.chooseIfNotGiven(cli, 'Storage account: ', 'Enumerating storage accounts', options.accountName,
          function(cb) {
            utils.doServiceManagementOperation(serviceManagementClient, 'listStorageAccounts', function (err, list) {
              var accountNames = list.body.map(function (st) {
          ...
          isAccountDefined = isNameDefined || isKeyDefined;
        }
        ...
        if (isAccountDefined) {
          ...
          if (isNameDefined && isKeyDefined) {
            connString = util.format('DefaultEndpointsProtocol=https;AccountName=%s;AccountKey=%s', options.accountName, options.accountKey);
            serviceClient = StorageUtil.getBlobService(connString);
          } else if (isNameDefined) {
            var tip = 'Retrieving storage account credentials';

            // Try to find key from the subscription
            startProgress(tip);
            try {
              var accountKeys = utils.doServiceManagementOperation(serviceManagementClient, 'getStorageAccountKeys', options.accountName, _);
              connString = util.format('DefaultEndpointsProtocol=https;AccountName=%s;AccountKey=%s', options.accountName, accountKeys.body.StorageServiceKeys.Primary);
              serviceClient = StorageUtil.getBlobService(connString);
            } catch (e) {
              throw new Error('Please set both --account-name and --account-key or specify an --account-name that belongs to the current subscription.');
            } finally {
              endProgress();
            }
          } else {
  ...
  function createServiceManagementClient(subscription) {
  ...
  */
  function listAzureContainersWithAcl(containerName, options, _) {
    ...
    var blobService = getBlobServiceClient(options, _);
    var listOperation = getStorageBlobOperation(blobService,'listAllContainers');
    var tips = 'Enumerating storage containers';
    ...
    interaction.formatOutput(cli, containers, function(outputData) {
      if(outputData.length === 0) {
        logger.info('No containers defined');
      } else {
  ...
  */
  function createAzureContainer(containerName, options, _) {
    ...
    var tips = util.format('Creating storage container %s', containerName);
  ...
  */
  function listAzureBlob(containerName, blobName, options, _) {
    ...
    var tips = 'Enumerating blobs in container ' + specifiedContainerName;
    ...
    interaction.formatOutput(cli, outputBlobs, function(outputData) {
      if(outputData.length === 0) {
        logger.info('No blobs defined');
      } else {
        logger.table(outputData, function (row, item) {
          row.cell('Name', item.name);
