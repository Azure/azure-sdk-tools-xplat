  ...
  function getBlobServiceClient(options, _) {
    ...
    if(isConnectionStringDefined && isAccountDefined) {
      ...
      if (isConnectionStringDefined) {
        ...
        if (isNameDefined) {
          if (!isKeyDefined) {
            var keysClient = cli.category('storage').category('account').category('keys');
            options.accountKey = keysClient.getStorageAccountKeys(options.accountName, options, _).Primary;
          }
