...
var StorageClient = require('./storage/storageclient');
  ...
  function getBlobServiceClient(options, _) {
    ...
    if(isConnectionStringDefined && isAccountDefined) {
      ...
      if (isConnectionStringDefined) {
        ...
        if (isNameDefined) {
          if (!isKeyDefined) {
            var storageClient = new StorageClient(cli, options.subscription);
            options.accountKey = storageClient.getStorageAccountKeys(options.accountName, _).Primary;
          }
