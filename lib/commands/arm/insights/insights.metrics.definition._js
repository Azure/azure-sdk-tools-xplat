...
exports.init = function (cli) {
  ...
  insightsMetricDefinitionCommand._prepareAndExecute = function (resourceId, options, _) {
    ...
    return this._executeCmd(client, resourceId, insightsUtils.passAllFilter, options, _);
  };

  insightsMetricDefinitionCommand._executeCmd = function (client, resourceId, keepTheRecord, options, _) {
    var progress = cli.interaction.progress(util.format($('Querying \"%s\"'), options.queryFilter));
    var result = [];
    try {
      var response = client.metricDefinitionOperations.getMetricDefinitions(resourceId, options.filter, _);
