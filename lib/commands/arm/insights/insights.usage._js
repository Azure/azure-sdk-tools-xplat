...
exports.init = function (cli) {
  ...
  insightsUsageCommand._processGeneralParameters = function (options) {
    var clauses = [];
  ...
  insightsUsageCommand._prepareAndExecute = function (resourceId, options, _) {
    ...
    this._processGeneralParameters(options);
    var apiVersion = options.apiVersion ? options.apiVersion : insightsUtils.defaultApiVersion;

    this._executeEventsCmd(client, resourceId, apiVersion, options, _);
  };

  insightsUsageCommand._executeEventsCmd = function (client, resourceId, apiVersion, options, _) {
    var progress = cli.interaction.progress(util.format($('Querying \"%s\"'), options.filter));
    var result = [];
    try {
      var response = client.usageMetrics.list(resourceId, apiVersion, options, _);
