...
exports.createLegacyInsightsClient = function(log, options) {
  var subscription = profile.current.getSubscription(options.subscription);
  var factoryMethod = require('./insightsLegacy').createLegacyInsightsClient;
  return utils.createLegacyInsightsClient(factoryMethod, subscription);
};
...
exports.removeRetentionPolicy = function (list) {
  var i;

  if (list) {
    for (i = 0; i < list.length; i++) {
      var element = list[i];
      delete element.retentionPolicy;
    }
  }
};
