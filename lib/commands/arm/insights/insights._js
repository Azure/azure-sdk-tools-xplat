...
exports.init = function (cli) {
  ...
  insightsEventsCommand.command('list')
      ...
      .option('-m --maxEvents <maxEvents>', $('The maximum number of records to be returned by the command.'))
      ...
      .execute(function (options, _) {
        ...
        if (presentSwitches > 1) {
          ...
          insightsCommand._prepareAndExecute(options.resourceId, 'resourceId', 'resourceId', options, _);
        } else {
  ...
  insightsCommand._executeEventsCmd = function (client, queryFilter, selectedFields, keepTheRecord, options, _) {
    ...
    try {
      ...
      var maxNumberOfRecords = __.isString(options.maxEvents) ? parseInt(options.maxEvents) : defaultMaxEvents;
      if (__.isNaN(maxNumberOfRecords) || maxNumberOfRecords < 1) {
        throw new Error(util.format($('Invalid value for argument %s: %s'), 'maxEvents', options.maxEvents));
      }
      ...
      while (nextLink && result.length < maxNumberOfRecords) {
      ...
      log.silly('result filtered #3: ' + result.length);
