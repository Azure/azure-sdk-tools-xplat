  ...
  var insightLogProfilesCommand = cli.category('insights').category('logprofile')
    ...
    .command('get <logProfileName>')
      ...
      .execute(function (logProfileName, options, _) {
        ...
        insightLogProfilesCommand._prepareAndExecute(options, _);
      });

  insightLogProfilesCommand._prepareAndExecute = function (options, _) {
    var client = insightsUtils.createInsightsManagementClient(log, options);
    var getResponse = client.logProfiles.get(options.logProfileName, _);
