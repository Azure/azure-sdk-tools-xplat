...
__.extend(VMClient.prototype, {
  ...
  createVMBackup: function (resourceGroupName, name, options, _) {
    ...
    result = virtualMachine.createOrUpdateVMExtension(resourceGroupName, name, azureVmBackupExtension.profile, true, _);
  ...
  setAzureDiskEncryption: function (resourceGroupName, name, options, _) {
    ...
    if (utils.ignoreCaseEquals(params.osType, 'Linux') &&
       !utils.ignoreCaseEquals(params.volumeType, 'Data')){
      throw new Error($('Enabling encryption is only allowed on Data volumes for Linux VMs.'));
    }
  ...
  disableAzureDiskEncryption: function(resourceGroupName, name, options, _) {
    ...
    if (utils.ignoreCaseEquals(params.osType, 'Linux')) {
      throw new Error($('Disabling encryption is only supported for Windows virtual machines'));
    }

    // Install extension
    var vmExtensionProfile = new VMExtensionProfile(this.cli, params);
    var disableAzureDiskEncryptionExtension = vmExtensionProfile.generateDisableAzureDiskEncryptionExtensionProfile();
    // Take user confirmation
    if (!options.quiet && 
    ...
    if(utils.ignoreCaseEquals(options.volumeType, 'All') ||
       utils.ignoreCaseEquals(options.volumeType, 'OS') ||
       (utils.stringIsNullOrEmpty(options.volumeType)))
    {
      //Update encryption settings
      var encryptionSettings = {
        enabled: false
