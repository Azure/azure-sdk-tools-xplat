...
__.extend(VMNetworkProfile.prototype, {
  generateNetworkProfile: function(_) {
    ...
    if (this.params.nicIds || this.params.nicNames) {
      ...
      networkProfile.profile.networkInterfaces.push({ id: nicInfo.profile.id });
    }
  ...
  _validateNICsById: function(_) {
    ...
    for (var i = 0; i < networkInterfaces.length; i++) {
      ...
      validNetworkInterfaces.push({ id: nicInfo.profile.id, primary: validNetworkInterfaces.length === 0 });
    }
