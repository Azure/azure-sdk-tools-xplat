...
exports.init = function (cli) {
  ...
  hostnames.command('delete [resource-group] [name] [hostnames]')
    ...
    .execute(function (resourceGroup, name, hostnames, options, _) {
      ...
      for (var i = 0; i < hostnamesToDeleteSplit.length; i++) {
        try {
          result = client.sites.deleteSiteHostNameBinding(resourceGroup, name, hostnamesToDeleteSplit[i], _);
        } finally {
          progress.end();
        }
      }
  ...
  container.command('set [resource-group] [name]')
    ...
    .option('-c --custom-image-name <customimagename>', $('the container custom image name and optionally the tag name'))
    .option('--slot <slot>', $('the name of the slot to show'))
    .option('-s --subscription <subscription>', $('the subscription identifier'))
    .execute(function (resourceGroup, name, options, _) {
      if (!resourceGroup) {
      ...
      for (var attrname in currentAppSettings.properties) { appSettingsProperties[attrname] = currentAppSettings.properties[attrname]; }

      if (options.url) {
        appSettingsProperties['DOCKER_REGISTRY_SERVER_URL'] = options.url;
      }
      if (options.username) {
        appSettingsProperties['DOCKER_REGISTRY_SERVER_USERNAME'] = options.username;
      }
      if (options.password) {
        appSettingsProperties['DOCKER_REGISTRY_SERVER_PASSWORD'] = options.password;
      }
      if (options.customimagename) {
        appSettingsProperties['DOCKER_CUSTOM_IMAGE_NAME'] = options.customimagename;
      }
      newAppSettings.properties = appSettingsProperties;
      try {
        result = client.sites.getSite(resourceGroup, name, _);
      } finally {
        progress.end();
      }
      newAppSettings.location = result.location;
      try {
        result = client.sites.updateSiteAppSettings(resourceGroup, name, newAppSettings, _);
      } finally {
        progress.end();
      }
  ...
  container.command('delete [resource-group] [name]')
    ...
    .execute(function (resourceGroup, name, options, _) {
      ...
      for (var attrname in appSettingsPrevious) {
        if (attrname != 'DOCKER_REGISTRY_SERVER_URL' &&
            attrname != 'DOCKER_REGISTRY_SERVER_USERNAME' &&
            attrname != 'DOCKER_REGISTRY_SERVER_PASSWORD' &&
            attrname != 'DOCKER_CUSTOM_IMAGE_NAME') {
          appSettingsProperties[attrname] = appSettingsPrevious[attrname];
        }
      }
      actualAppSettings.properties = appSettingsProperties;
      try {
