...
exports.init = function (cli) {
  ...
  config.command('show [resource-group] [name]')
    .description($('Get webapp configuration \nexample:  webapp config show resroucegroup1 webapp1\nexample:  webapp config show resroucegroup1 webapp1 --propertiesToInclude remoteDebuggingVersion,logsDirectorySizeLimit '))
    ...
    .execute(function (resourceGroup, name, options, _) {
      ...
      cli.interaction.formatOutput(publishingResult, function (data) {
        if (!data) {
          ...
          log.data($('Publishing Username            :'), data.publishingUserName);
          log.data($('Publishing Password            :'), data.publishingPassword);
          log.data('');
        }
      });
      ...
      cli.interaction.formatOutput(result, function (data) {
        if(options.propertiesToInclude !== null && options.propertiesToInclude !== undefined) {
            var extraProperties = (options.propertiesToInclude).split(',', 100);
            log.data($('Extra Properties'));
            log.data($('--------------------------------'));
            for(var i = 0; i < extraProperties.length; i++) {
              log.data($(extraProperties[i] + ': '), data[extraProperties[i]]);
            }
            log.data($(''));
          }
      });
  ...
  config.command('set [resource-group] [name]')
    ...
    .usage('[options] <resource-group> <name>\nexample:  webapp config set resroucegroup1 webapp1 --alwayson true --numberofworkers 1')
    .option('-g --resource-group <resource-group>', $('the name of the resource group'))
    ...
    .option('--numberofworkers <numberofworkers>', $('node version of webapp'))
    .option('--netframeworkversion <netframeworkversion>', $('node version of webapp'))
    .option('--requesttracingenabled <requesttracingenabled>', $('node version of webapp'))
    .option('--remotedebuggingenabled <remotedebuggingenabled>', $('node version of webapp'))
    .option('--httploggingenabled <httploggingenabled>', $('node version of webapp'))
    .option('--detailederrorloggingenabled <detailederrorloggingenabled>', $('node version of webapp'))
    .option('--websocketenabled <websocketenabled>', $('node version of webapp'))
    .option('--use32bitworkerprocess <use32bitworkerprocess>', $('node version of webapp'))
    .option('--alwayson <alwayson>', $('node version of webapp'))
    .option('--autohealenabled <autohealenabled>', $('node version of webapp'))
  ...
  function toBool(stringToParse) {
   ...
   appsetting.command('add [resource-group] [name] [appsettings]')
    .description($('Add webapp app settings (using comma seperated key value pairs)\nexample:  webapp config appsetting add KEY1=val1,KEY2=val2,KEY3=val3'))
    ...
    .execute(function (resourceGroup, name, appsettings, options, _) {
      ...
      cli.interaction.formatOutput(currentAppSettings.properties, function (data) {
        log.data($('Current App Settings'));
        log.data($('--------------------------------'));
        if(currentAppSettings.properties !== null && currentAppSettings.properties !== undefined) {
          log.data(data);
          log.data($(''));
        }
        else {
          log.data($('No app settings set'));
        }
      });
      ...
      for(var i = 0; i < appSettingsToAddSplit.length; i++) {
        settingSplit = appSettingsToAddSplit[i].split('=');
