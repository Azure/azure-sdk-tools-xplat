...
exports.getSizeUserSpecific = function getSizeInUserSpecificFormat(skuFamily,skuCapacity){
	var sizeConstant = skuFamily + skuCapacity;
	switch(sizeConstant)
	{
		case C0String:
				return C0;
...
exports.parseBoolean = function(argName, argValue, _default) {
  if (__.isUndefined(argValue)) {
    return _default;
  }
...
exports.showNotFoundError = function notFoundError(resourceGroup, cacheName) {
    var msg;
    if (resourceGroup) {
      msg = util.format($('Cache not found on resource group %s: %s'), resourceGroup, cacheName);
    } else {
