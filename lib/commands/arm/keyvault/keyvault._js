...
exports.init = function(cli) {
  ...
  function getAccountBySPN(spn, _) {
    ...
    var accounts = graphClient.servicePrincipalOperations.list(parameters, _);
  ...
  function getAccountByUPN(upn, _) {
    ...
    var accounts = graphClient.userOperations.list(parameters, _);
    if (!accounts || accounts.length === 0) {
  ...
  function resolvePrincipalName(accessPolicy, _) {
    ...
    try {
      var servicePrincipal = graphClient.servicePrincipalOperations.get(accessPolicy.objectId, _);
      if (servicePrincipal) {
    ...
    try {
      var user = graphClient.userOperations.get(accessPolicy.objectId, _);
      if (user) {
