...
exports.init = function (cli) {
  ...
  localNetworkGateways.command('create [resource-group] [name] [location] [ip-address]')
    ...
    .execute(function(resourceGroup, name, location, ipAddress, options, _) {
      ...
      try {
        result = networkManagementClient.localNetworkGateways.get(resourceGroup, name, null, _);
      } catch (e) {
        if (e.statusCode === 404) {
          result = null;
        } else {
      ...
      try {
        result = networkManagementClient.localNetworkGateways.createOrUpdate(resourceGroup, name, parameters, _);
      } finally {
        progress.end();
      }
    ...
    .execute(function(resourceGroup, name, options, _) {
      ...
      if(options.addressSpace) {
        if(!utils.argHasValue(options.addressSpace)) {
          ...
          if(result && result.localNetworkAddressSpace && result.localNetworkAddressSpace.addressPrefixes) {
            parameters.localNetworkAddressSpace.addressPrefixes = result.localNetworkAddressSpace.addressPrefixes;
          } else {
            parameters.localNetworkAddressSpace.addressPrefixes = [];
          }
          parameters.localNetworkAddressSpace.addressPrefixes = parameters.localNetworkAddressSpace.addressPrefixes.concat(options.addressSpace.split(','));
  ...
  localNetworkGateways.command('delete [resource-group] [name]')
    ...
    .execute(function(resourceGroup, name, options, _) {
      ...
      try {
        result = networkManagementClient.localNetworkGateways.deleteMethod(resourceGroup, name, _);
    ...
    .execute(function(resourceGroup, options, _) {
      ...
      try {
        if(typeof networkManagementClient.localNetworkGateways.listAll != 'function') {
          ...
          result = networkManagementClient.localNetworkGateways.list(resourceGroup,  _);
        } else {
          if(options.resourceGroup) {
            ...
            progress = cli.interaction.progress($('Getting the local network gateways'));
            result = networkManagementClient.localNetworkGateways.listAll(_);
          }
        }
      } finally {
        progress.end();
