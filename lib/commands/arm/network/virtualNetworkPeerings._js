...
exports.init = function (cli) {
  ...
  virtualNetworkPeerings.command('create [resource-group] [vnet-name] [name]')
    ...
    .option('--remote-vnet-id <remote-vnet-id>', $('the reference of the remote virtual network by Id'))
    .option('--allow-forwarded-traffic <allow-forwarded-traffic>', $('whether the forwarded traffic from the VMs in the remote virtual network will be allowed/disallowed'))
    .option('--allow-gateway-transit <allow-gateway-transit>', $('if gatewayLinks can be used in remote virtual network’s link to this virtual network'))
    .option('--use-remote-gateways <use-remote-gateways>', $('if remote gateways can be used on this virtual network. If the flag is set to true, and allowGatewayTransit on remotepeering is also true, virtual network will use gateways of remote virtual network for transit. Only 1 peering can have this flag set to true. This flag cannot be set if virtual network already has a gateway.'))
    .option('--allow-vnet-access <allow-vnet-access>', $('whether the VMs in the linked virtual network space would be able to access all the VMs in local Virtual network space'))
