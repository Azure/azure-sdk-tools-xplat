...
__.extend(VirtualNetworkGateway.prototype, {
  ...
  _showGateway: function (gateway, resourceGroupName, gatewayName) {
    ...
    self.interaction.formatOutput(gateway, function (gateway) {
      ...
      if (gateway.vpnClientConfiguration && gateway.vpnClientConfiguration.vpnClientAddressPool) {
        self.output.header($('Address Prefixes'));
        var addressPrefixes = gateway.vpnClientConfiguration.vpnClientAddressPool.addressPrefixes;
        self.output.list(addressPrefixes, 2);
      }
