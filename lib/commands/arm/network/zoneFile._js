...
__.extend(ZoneFile.prototype, {
  ...
  parseRR: function (rr, $origin, zoneName, $ttl, prevRRname) {
    ...
    // support CNAME records with numeric names
    if (!res.name && res.type === 'CNAME') {
      res.name = res.ttl.toString();
      delete res.ttl;
    }
  ...
  parseSOA: function (rr, $origin) {
    var self = this;
    console.log('PARSE SOA RR: %j', rr);
    console.log('SN: ' + self.parseTimestamp(rr.data[2]));
    var soa = {
      name: rr.name,
      type: rr.type,
      ttl: rr.ttl,
  ...
  parseTXT: function (rr) {
    ...
    // Where multiple strings are given, they should be concatenated into a single string (without adding any whitepace).
    var value = '';
    ...
    // If the resulting string exceeds the maximum length of a single string (255 characters), it shall be truncated and a warning given.
    if (value.length > constants.dnsZone.txtStringMaxLength) {
      value = value.substr(0, constants.dnsZone.txtStringMaxLength);
  ...
  generateRR: function (name, type, ttl, record) {
    ...
    switch (type) {
      case constants.dnsZone.SOA:
        console.log('SERIAL NUMBER BEFORE '+record.serialNumber);
        var serialNumber = record.serialNumber || moment().format('YYYYMMDDss');
        console.log('SERIAL NUMBER AFTEr'+ serialNumber);
