...
__.extend(AppGateways.prototype, {
  create: function (resourceGroup, appGatewayName, location, options, _) {
    ...
    try {
      var createdAppGateway = self.networkManagementClient.applicationGateways.createOrUpdate(resourceGroup, appGatewayName, appGateway, _);
    } finally {
      progress.end();
    }
  ...
  list: function (options, _) {
    ...
    try {
      if (options.resourceGroup) {
        var progress = self.interaction.progress(util.format($('Looking up application gateways in resource group "%s"'), options.resourceGroup));
        appGateways = self.networkManagementClient.applicationGateways.list(options.resourceGroup, options, _);
      } else {
        var progress = self.interaction.progress($('Looking up application gateways in all resource groups'));
  ...
  setIpConfig: function (resourceGroup, appGatewayName, name, options, _) {
    ...
    if (subnetId.toLowerCase() != ipConfig.subnet.id.toLowerCase()) {
      ipConfig.subnet = {id: subnetId};
      appGateway.gatewayIPConfigurations = [ipConfig];
      self._setAppGateway(resourceGroup, appGatewayName, appGateway, _);
    } else {
      throw new Error(util.format($('Current gateway ip config already has the same subnet with Id: "%s"'), subnetId));
    }
  },
  ...
  addProbe: function (resourceGroup, appGatewayName, probeName, options, _) {
    ...
    var probe = self._parseProbe(resourceGroup, appGatewayName, probeName, options, _);
  ...
  _parseFrontendIp: function (resourceGroup, appGatewayName, frontendIpName, options, _) {
    var self = this;
    var frontendIp = {
      id: self._getResourceId(resourceGroup, appGatewayName, 'frontendIPConfigurations', frontendIpName),
      name: frontendIpName
    };
  ...
  _parseProbe: function(resourceGroup, appGatewayName, probeName, options, _) {
    var self = this;
    var probe = {
      name: probeName
    };
    ...
    if (options.hostName && options.path) {
      ...
      self.output.warn(util.format($('Using default probe URL: %s%s'), constants.appGateway.probe.host,
        constants.appGateway.probe.path));
    ...
    if (options.unhealthyThreshold) {
      ...
      self.output.warn(util.format($('Using default unhealthy threshold: %s'),
        constants.appGateway.probe.unhealthyThreshold));
  ...
  _showAppGateway: function (appGateway) {
    ...
    self.interaction.formatOutput(appGateway, function (appGateway) {
      console.log('PROBES: %j', appGateway.probes);
