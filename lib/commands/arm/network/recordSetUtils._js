...
exports.covertToAzureFormat = function (recordSet) {
  console.log('recordset %j', recordSet);
  var parameters = {
    recordSet: {
      name: recordSet.name,
  ...
  console.log('parameters in converter %j', parameters);
  ...
  switch (recordSet.type) {
    case constants.dnsZone.SOA :
      ...
      //delete parameters.recordSet.properties.soaRecord.host;
      break;
    ...
    case constants.dnsZone.NS :
      ...
      if (parameters.recordSet.name === '@') delete parameters.recordSet.properties.nsRecords;
      break;
  ...
  for (var i = 0; i < recordSets.length; i++) {
    ...
    if (!__.isEmpty(recordSet.properties.soaRecord)) {
      ...
      items.push({
        ...
        metadata: recordSet.properties.metadata,
        record: 'host:    ' + soaRecord.host
      });
      items.push({record: 'email:   ' + soaRecord.email});
      items.push({record: 'serial:  ' + soaRecord.serialNumber});
