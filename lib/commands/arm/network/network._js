...
exports.init = function (cli) {
  ...
  appGateway.command('create [resource-group] [name] [location]')
    ...
    .option('-i, --http-settings-protocol <http-settings-protocol>', util.format($('the HTTP settings protocol, valid values are \[%s\]'),
    ...
    .option('-w, --routing-rule-type <routing-rule-type>', util.format($('the request routing rule type, default is "%s"'), constants.appGateway.routingRule.type[0]))
    .option('-a, --sku-name <sku-name>', util.format($('the name of the sku, valid values are \[%s\]. Default values is "%s"'),
      constants.appGateway.sku.name, constants.appGateway.sku.name))
    .option('-t, --sku-tier <sku-tier>', util.format($('the sku tier, valid values are \[%s\]. Default values is "%s"'),
      constants.appGateway.sku.name.tier, constants.appGateway.sku.name.tier))
    ...
    .execute(function (resourceGroup, name, location, options, _) {
      ...
      options.subnetName = cli.interaction.promptIfNotGiven($('Virtual network subnet name: '), options.subnetName, _);
  ...
  appGatewayIpConfig.command('set [resource-group] [gateway-name] [name]')
    .description($('Update an ip config to an application gateway'))
    .usage('[options] <resource-group> <gateway-name> <name>')
    .option('-g, --resource-group <resource-group>', $('the name of the resource group'))
  ...
  appGatewayHttpSettings.command('remove [resource-group] [gateway-name] [name]')
    .description($('Remove http settings to an application gateway'))
    .usage('[options] <resource-group> <gateway-name> <name>')
    .option('-g, --resource-group <resource-group>', $('the name of the resource group'))
    .option('-n, --gateway-name <gateway-name>', $('the name of the application gateway'))
    .option('-b, --name <name>', $('the name of the HTTP settings'))
