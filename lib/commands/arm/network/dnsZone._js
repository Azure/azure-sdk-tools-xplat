...
__.extend(DnsZone.prototype, {
  ...
  createRecordSet: function (resourceGroupName, zoneName, setName, options, _) {
    ...
    /* if (options.type === constants.dnsZone.CNAME) {
     throw new Error(util.format($('A record set of type "%s" can not be created directly, use "add-record" command instead'), options.type));
     }*/
    ...
    if (options.type === constants.dnsZone.CNAME) {
      delete recordSet.recordSet.properties.cnameRecord;
    }
  ...
  deleteRecordSet: function (resourceGroupName, zoneName, setName, options, _) {
    ...
    var result;
    try {
      self.dnsManagementClient.recordSets.deleteMethod(resourceGroupName, zoneName, setName, options.type, _);
    } finally {
      progress.end();
    }
  },
  ...
  addRecord: function (resourceGroupName, zoneName, setName, options, _) {
    ...
    if (options.type === constants.dnsZone.CNAME) {
      ...
      // it's not permitted to update existing CNAME record
      recordSet = self.getRecordSet(resourceGroupName, zoneName, setName, options.type, _);
      if (recordSet) {
        throw new Error(util.format($('A record set with name "%s" of type "%s" can not be modified'), setName, options.type));
      }

      // CNAME record sets can be created only in time of creation record itself
      recordSet = {
        recordSet: {
          properties: {
            ttl: 3600
      ...
      // all other types of record sets should be already created to add/remove records
      recordSet = self.getRecordSet(resourceGroupName, zoneName, setName, options.type, _);
      if (!recordSet) {
        throw new Error(util.format($('A record set with name "%s" of type "%s" not found in the resource group "%s"'), setName, options.type, resourceGroupName));
      }
  ...
  _parseZone: function (zone, options) {
    ...
    console.log('PARAMS ZONE %j', parameters);
    return parameters;
  },
