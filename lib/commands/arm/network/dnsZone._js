...
__.extend(DnsZone.prototype, {
    ...
    try {
      if (options.resourceGroup) {
        ...
        while (nextLink !== undefined) {
          ...
          var nextZones = self.dnsManagementClient.zones.listInResourceGroupNext(nextLink, options, _);
        ...
        while (nextLink !== undefined) {
          self.output.silly('Following nextLink');
          var nextZones = self.dnsManagementClient.zones.listInSubscriptionNext(nextLink, options, _);
          dnsZones.zones = dnsZones.concat(nextZones.zones);
          nextLink = nextZones.nextLink;
        }
      }
  ...
  delete: function (resourceGroupName, zoneName, options, _) {
    ...
    if (!self.interaction.confirm(util.format($('WARNING: This will delete the DNS zone "%s" and all DNS records. ' +
    ...
    self.interaction.formatOutput(dnsRecords, function (outputData) {
      if (outputData.length === 0) {
        ...
        formattedRecordSetList.forEach(function (recordSet) {
          ...
          self.output.nameValue($('Records'), recordSet.record);
  ...
  _parseRecord: function (recordSet, options, isAdding) {
    ...
    if (options.text) {
      if (options.type !== constants.dnsZone.TXT) {
        ...
        var textArray = self._splitTxtRecord(options.text);
        if (isAdding) {
          recordSet.txtRecords.push({value: textArray});
        } else {
          recordIndex = utils.indexOfCaseIgnore(recordSet.txtRecords, {value: textArray});
          if (recordIndex === -1) {
  ...
  _showZone: function (zone, resourceGroupName, zoneName) {
    ...
    self.interaction.formatOutput(zone, function (zone) {
      ...
      self.output.nameValue($('Metadata'), tagUtils.getTagsInfo(zone.metadata));
    });
  },
