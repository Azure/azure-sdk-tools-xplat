...
__.extend(ExpressRoute.prototype, {
   ...
   */
  createAuthorization: function (resourceGroupName, circuitName, name, options, _) {
    var self = this;
    var circuit = self.getCircuit(resourceGroupName, circuitName, _);
    if (!circuit) {
      throw new Error(util.format($('An express route circuit with name "%s" not found in the resource group "%s"'), circuitName, resourceGroupName));
    }
    var circuitAuth = self.getAuthorization(resourceGroupName, circuitName, name, _);
    if (circuitAuth) {
    ...
    circuitAuth = self._parseCircuitAuthorization(circuitAuth, options, true);
    var progress = self.interaction.progress(util.format($('Creating express route circuit authorization "%s" in circuit "%s"'), name, circuitName));
    try {
      self.networkManagementClient.expressRouteCircuitAuthorizations.createOrUpdate(resourceGroupName, circuitName, name, circuitAuth, _);
    } finally {
      progress.end();
    }
    self.showAuthorization(resourceGroupName, circuitName, name, options, _);
  },

  setAuthorization: function (resourceGroupName, circuitName, name, options, _) {
    var self = this;
    var circuit = self.getCircuit(resourceGroupName, circuitName, _);
    if (!circuit) {
      throw new Error(util.format($('An express route circuit with name "%s" not found in the resource group "%s"'), circuitName, resourceGroupName));
    ...
    circuitAuth = self._parseCircuitAuthorization(circuitAuth, options, false);
    var progress = self.interaction.progress(util.format($('Setting express route circuit authorization "%s"in circuit "%s"'), name, circuitName));
    try {
      self.networkManagementClient.expressRouteCircuitAuthorizations.createOrUpdate(resourceGroupName, circuitName, name, circuitAuth, _);
    } finally {
      progress.end();
  ...
  showAuthorization: function (resourceGroupName, circuitName, name, options, _) {
    var self = this;
    var circuit = self.getCircuit(resourceGroupName, circuitName, _);
    if (!circuit) {
      throw new Error(util.format($('An express route circuit with name "%s" not found in the resource group "%s"'), circuitName, resourceGroupName));
  ...
  deleteAuthorization: function (resourceGroupName, circuitName, name, options, _) {
    var self = this;
    var circuit = self.getCircuit(resourceGroupName, circuitName, _);
    if (!circuit) {
      throw new Error(util.format($('An express route circuit with name "%s" not found in the resource group "%s"'), circuitName, resourceGroupName));
    ...
    if (!options.quiet && !self.interaction.confirm(util.format($('Delete express route circuit authorization "%s" in circuit "%s"? [y/n] '), name, circuitName), _)) {
      return;
    }

    var progress = self.interaction.progress(util.format($('Deleting express route circuit authorization "%s" in circuit "%s"'), name, circuitName));
    try {
      self.networkManagementClient.expressRouteCircuitAuthorizations.deleteMethod(resourceGroupName, circuitName, name, null, _);
    } finally {
      progress.end();
    }
  },
  ...
  getAuthorization: function (resourceGroupName, circuitName, name, _) {
    var self = this;
    var progress = self.interaction.progress(util.format($('Looking up the express route circuit authorization "%s" in circuit "%s"'), name, circuitName));
    try {
      var circuit = self.networkManagementClient.expressRouteCircuitAuthorizations.get(resourceGroupName, circuitName, name, null, _);
      return circuit;
    } catch (e) {
      if (e.statusCode === 404) {
        return null;
  ...
  _parseCircuitAuthorization: function (circuitAuth, options, useDefaults) {
    var self = this;
    if (options.key) {
      circuitAuth.authorizationKey = options.key.toString('base64');
    }
    if (options.status) {
      circuitAuth.authorizationUseStatus = utils.verifyParamExistsInCollection(constants.expressRoute.authorizationStatus, options.status, '--status');
    } else {
      if (useDefaults) {
        circuitAuth.authorizationUseStatus = constants.expressRoute.authorizationStatus[0];
        self.output.warn(util.format($('Using default authorization use status: %s'), circuitAuth.authorizationUseStatus));
      }
    }
    return circuitAuth;
  }
