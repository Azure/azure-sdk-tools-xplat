  ...
  deployment.command('create [resource-group] [name]')
    ...
    .execute(function (resourceGroup, name, options, _) {
      ...
      cli.interaction.withProgress($('Waiting for deployment to complete'),
        function (log, _) {          
          do {
            d = client.deployments.get(resourceGroup, deployment.name, _).deployment;            
          } while (d.properties.provisioningState == 'Running' || d.properties.provisioningState == 'Accepted');         

          if (d.properties.provisioningState != 'Succeeded') {
            console.log('Deployment provisioning state was not successful. State is %s', d.properties.provisioningState);            
          }
      ...
      displayDeployment(d, resourceGroup, true, cli.output);
    });
