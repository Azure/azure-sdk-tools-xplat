...
exports.init = function (cli) {
  ...
  site.command('list')
        ...
        .execute(function (options, _) {
          ...
          try {
            locations = service.webSpaces.list(_).webSpaces;
  ...
  function portalGitInitInstruction(context, _) {
    ...
    if (confirm($('Launch browser to portal now? [y/n] '), _)) {
      ...
      cli.interaction.launchBrowser(href);
    }
  }
      ...
      for (var property in siteData.siteProperties.properties) {
        if (utils.ignoreCaseEquals(property, 'RepositoryUri')) {
          if (typeof siteData.siteProperties.properties[property] === 'string') {
            if (!endsWith(siteData.siteProperties.properties[property], '/')) {
  ...
  site.doSitesGet = function (options, _) {
    ...
    if (!spaces) {
      ...
      try {
        spaces = service.webSpaces.list(_).webSpaces;
      } finally {
        progress.end();
      }
    }
  ...
  function createWebsiteManagementService(subscription) {
    return utils._createWebsiteManagementService(cli.category('account').getCurrentSubscription(subscription), log);
  }
  ...
  function getHostNameSuffix(subscription, _) {
    ...
    if (subscriptionId) {
      ...
      // TODO: fix this so that '_' is not required
      return hostNameSuffix.DnsSuffix['_'] || cli.environmentManager.getHostNameSuffix();
    } else {
