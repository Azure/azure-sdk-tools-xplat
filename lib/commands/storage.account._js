...
exports.init = function (cli) {
  ...
  storageAccount.listCommand = function (options, callback) {
      ...
      var progress = cli.progress($('Getting storage accounts'));
  ...
  storageAccount.showCommand = function (name, options, callback) {
      ...
      var progress = cli.progress($('Getting storage account'));
  ...
  storageAccount.deleteCommand = function (name, options, _) {
      ...
      if (!options.quiet && !interaction.confirm(cli, util.format($('Delete storage account %s? [y/n] '), name), _)) {
        return;
      }
  ...
  keys.getStorageAccountKeys = function (name, options, _) {
    var channel = utils.createServiceManagementService(cli.category('account').lookupSubscriptionId(options.subscription),
        cli.category('account'), log);

    var progress = cli.progress($('Getting storage account keys'));
    try {
      var response = utils.doServiceManagementOperation(channel, 'getStorageAccountKeys', name, _);
  ...
  keys.listCommand = function (name, options, _) {
    var storageKeys = keys.getStorageAccountKeys(name, options, _);
    interaction.formatOutput(cli, storageKeys, function (outputData) {
      if (outputData) {
