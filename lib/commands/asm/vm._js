...
exports.init = function(cli) {
  ...
  vm.command('create <dns-name> <image> <user-name> [password]')
    ...
    .option('-z, --vm-size <size>', $('the virtual machine size [small]\n    extrasmall, small, medium, large, extralarge, a5, a6, a7, basic_a0, basic_a1, basic_a2, basic_a3, basic_a4\n    Also supported: a0, a1, a2, a3, a4'))
	...
	// Allowed values are:
	//   'ExtraSmall,Small,Medium,Large,ExtraLarge,A5,A6,A7,Basic_A0,Basic_A1,Basic_A2,Basic_A3,Basic_A4'
        // Also accepting A0,A1,A2,A3,A4 as synonyms of ExtraSmall,Small,Medium,Large,ExtraLarge
	if (vmSize === 'extrasmall' || vmSize === 'a0') {
          vmSize = 'ExtraSmall';
        } else if (vmSize === 'small' || vmSize === 'a1') {
          vmSize = 'Small';
        } else if (vmSize === 'medium' || vmSize === 'a2') {
          vmSize = 'Medium';
        } else if (vmSize === 'large' || vmSize === 'a3') {
          vmSize = 'Large';
        } else if (vmSize === 'extralarge' || vmSize === 'a4') {
          vmSize = 'ExtraLarge';
        } else if (vmSize.slice(0,7) === 'basic_a') {
          vmSize = 'Basic_A' + vmSize[7];
        } else {
          vmSize = vmSize[0].toUpperCase() + vmSize.slice(1);
        }

	logger.verbose('vmSize='+vmSize);

        if (vmSize !== 'ExtraSmall' && vmSize !== 'Small' && vmSize !== 'Medium' &&
            vmSize !== 'Large' && vmSize !== 'ExtraLarge' &&
            vmSize !== 'A5' && vmSize !== 'A6' && vmSize !== 'A7' &&
            vmSize !== 'Basic_A0' && vmSize !== 'Basic_A1' && vmSize !== 'Basic_A2' &&
            vmSize !== 'Basic_A3' && vmSize !== 'Basic_A4') {
          logger.help('--vm-size <size> must specify one of the following:');
          logger.help('  extrasmall, small, medium, large, extralarge, a5, a6, a7, basic_a0, basic_a1, basic_a2, basic_a3, basic_a4');
          logger.help('  Also supported: a0, a1, a2, a3, a4');
  ...
  location.command('list')
    ...
    .execute(function (options, _) {
      var service = utils._createManagementClient(profile.current.getSubscription(options.subscription), logger);
  ...
  function loadCustomData(udfile) {
    if (udfile) {
  ...
  function createVM(options, cmdCallback) {
	...
	if (options.customData) {
          ...
          if (fs.statSync(options.customData)['size'] < Constants.CUSTOM_DATA_FILE_SIZE) {
            customDataBase64 = loadCustomData(options.customData);
            if (customDataBase64) {
              provisioningConfig.CustomData = customDataBase64;
            }
          } else {
            var fileErr = $('Input custom data file exceeded the maximum length of ' + Constants.CUSTOM_DATA_FILE_SIZE + ' bytes');
            return cmdCallbackHook(fileErr);
          }
        }
              ...
              utils.doServiceManagementOperation(channel, 'listAffinityGroups', function(error, affinityGrpRes) {
                ...
                if (!error) {
                  ...
                  if (foundAffinityGroup.Capabilities && foundAffinityGroup.Capabilities.Capability &&
                    !(foundAffinityGroup.Capabilities.Capability instanceof Array)) {
                    // Normalize Capability to an array.
                    foundAffinityGroup.Capabilities.Capability = [foundAffinityGroup.Capabilities.Capability];
                  }
  ...
  function deleteVM(name, options, _) {
    ...
    if (found) {
      ...
      var done = function () {
        ...
        logger.verbose(util.format($('All %s disk(s) were successfully deleted from disk registry and blob storage'), allCount));
      };
    } else {
          ...
          for(var j=0; j <configSet.InputEndpoints.length; j++) {
            if(configSet.InputEndpoints[j].Name === endpointName) {
              endpointSet = {
                ...
                Vip : configSet.InputEndpoints[j].Vip,
