...
exports.init = function(cli) {
  ...
  endpoint.command('update <vm-name> <endpoint-name>')
    ...
    .option('-a, --acl-file <file>', $('the json file containing the EndpointAcl definition'))
    .option('-s, --subscription <id>', $('the subscription id'))
    .execute(function(vmName, endpointName, options, callback) {
      var endPointUtil = new EndPointUtil();
      var endPoint = {};
      ...
      if(options.aclFile) {
          var jsonFile = fs.readFileSync(options.aclFile, 'utf8');
          var aclList = JSON.parse(jsonFile);
          if(aclList.Rules){
              endPoint.EndpointAcl = aclList;
          }else{
              return callback($('the acl file provided is malformed'));
          }
      }
        ...
        if (result.error) {
          ...
          utils.doServiceManagementOperation(channel, 'getRole', result.deployment.svc,
            ...
            if (error) {
              ...
              if (epInput.EndpointAcl){
                endPoint.EndpointAcl = epInput.EndpointAcl;
              }
