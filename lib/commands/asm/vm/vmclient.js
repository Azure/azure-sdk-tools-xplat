...
_.extend(VMClient.prototype, {
  ...
  uploadDataDisk: function(sourcePath, blobUrl, storageAccountKey, options, callback, logger) {
    ...
    if (/^https?\:\/\//i.test(sourcePath)) {
      ...
      pageBlob.copyBlob(sourcePath, options.sourceKey, blobUrl, storageAccountKey, function(error, blob, response) {
        ...
        if (!error) {
          logger.silly('Status : ' + blob.copy.status);
        }
        ...
        if (extensions.length === 0) {
          ...
          var compressText = function (str) {
            ...
            if (str.length > maxTxtLength) {
              str = str.substring(0, maxTxtLength) + '...';
            }
          ...
          for (var index in extensions) {
            ...
            for(var key in item) {
              ...
              self.logger.nameValue(normalizeStr(key), compressText(value));
            }
            self.logger.data('');
          }
        }
  ...
  setChefExtension: function(vmName, options, callback) {
    ...
    // validation.pem or client.pem and client-config are mandatory
    if ( (!options.validationPem && !options.clientPem) || !options.clientConfig) {
      return callback(
        new Error($('Required --validation-pem or --client-pem and --client-config options.')));
    }
    ...
    self.getDeployments(options, function(error, deployments) {
      ...
      if (options.clientPem) {
        extnPropFrom = 'clientPem';
      } else if (options.validationPem) {
        extnPropFrom = 'validationPem';
      }

      async.series([
        _.bind(loadConfig, self, options, 'clientConfig', 'publicConfig'),
        _.bind(loadConfig, self, options, extnPropFrom, 'privateConfig'),
        function configureExtension(cb) {
          // add this extension to the role if this is a new extension
          if (!extension) {
            extension = {};
        ...
        fs.readFile(options[propFrom], function(err, data) {
          if (!err) {
            ...
            if (propTo === 'publicConfig') {
              ...
              if (propFrom === 'validationPem') {
                config['validation_key'] = data.toString();
                options[propTo] = JSON.stringify(config);
              } else if (propFrom === 'clientPem') {
                config['client_pem'] = data.toString();
                options[propTo] = JSON.stringify(config);
              }
            }
          }
...
function createVMView(role, deployment, logger) {
  ...
  try {
    ...
    if (logger) {
      var message = util.format($('Failed to parse role and deployment with the following exception: \'%s\'.'), ex);
      logger.warn(message);
    }
  }
