...
// TODO: this self definition along with the assignment just bellow
// is an extremely nasty hack that should be removed ASAP once proper 
// usage and definitions of self are found in this file.
var self;
...
function getVMSize(options, logger, callback) {
  ...
  if (options.vmSize) {
    ...
    if (vmSize.length == 8 && vmSize.slice(0,5) === 'basic') {
      vmSize = vmSize[0].toUpperCase() + vmSize.slice(1,6) + vmSize[6].toUpperCase() + vmSize[7];
    } else if (vmSize.length > 5 && vmSize.slice(0,5) === 'extra') {
      vmSize = vmSize[0].toUpperCase() + vmSize.slice(1,5) + vmSize[5].toUpperCase() + vmSize.slice(6);
    } else {
    ...
    if (vmSize !== 'ExtraSmall' && vmSize !== 'Small' && vmSize !== 'Medium' &&
      ...
      vmSize !== 'A5' && vmSize !== 'A6' && vmSize !== 'A7' && vmSize !== 'A8' && vmSize !== 'A9' &&
      vmSize !== 'Basic_A0' && vmSize !== 'Basic_A1' && vmSize !== 'Basic_A2' && vmSize !== 'Basic_A3' && vmSize !== 'Basic_A4') {
      logger.help('--vm-size <size> must specify one of the following:');
      logger.help('  extrasmall, small, medium, large, extralarge, a5, a6, a7, a8, a9, basic_a0, basic_a1, basic_a2, basic_a3, basic_a4  ');
