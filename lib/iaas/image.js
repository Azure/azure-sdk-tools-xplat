...
var VMClient = require('../commands/vm/vmclient');
...
exports.imageDelete = function(what, cli) {
  return function(name, options, callback) {
    ...
    deleteImage.deleteImage(whatAPI[what], ['Disk', 'VM image'][what], logger, channel, name, null, cli.interaction.progress, options.blobDelete, callback);
  };
};
    ...
    function createImage(finalBlobUrl) {
      ...
      if (what === DISK) {
        utils.doServiceManagementOperation(channel, 'addDisk', name,
            normUrl,
            options2.imageOptions, function(error) {
          callback(error);
        });
      } else {
