...
describe('arm', function () {
  describe('network', function () {
    ...
    describe('vnet peering', function () {
      this.timeout(hour);
      it('create should create vnet peering', function (done) {
        networkUtil.createGroup(groupName, location, suite, function () {

          var cmd = ('network vnet create -g {group} -n virtualNetworkName  ' +
            '--location westus --json').formatArgs(virtualNetworkPeerings);

          testUtils.executeCommand(suite, retry, cmd, function (virtualNetwork) {
            virtualNetwork.exitStatus.should.equal(0);
            var cmd = ('network vnet create -g {group} -n virtualNetworkNameRemote --address-prefixes 42.0.0.0/8 ' +
              '--location westus --json').formatArgs(virtualNetworkPeerings);
            testUtils.executeCommand(suite, retry, cmd, function (virtualNetworkRemote) {
              virtualNetworkRemote.exitStatus.should.equal(0);
              virtualNetworkRemote = JSON.parse(virtualNetworkRemote.text);
              virtualNetworkPeerings.remoteVnetId = virtualNetworkRemote.id;
              var cmd = 'network vnet peering create -g {group} -n {name} --vnet-name virtualNetworkName --remote-vnet-id {remoteVnetId} --allow-forwarded-traffic {allowForwardedTraffic} --allow-gateway-transit {allowGatewayTransit} --allow-vnet-access {allowVirtualNetworkAccess} --json'.formatArgs(virtualNetworkPeerings);
      ...
      it('show should display vnet peering details', function (done) {
      ...
      it('set should update vnet peering', function (done) {
        var cmd = 'network vnet peering set -g {group} -n {name} --vnet-name virtualNetworkName --allow-forwarded-traffic {allowForwardedTrafficNew} --allow-gateway-transit {allowGatewayTransitNew} --allow-vnet-access {allowVirtualNetworkAccessNew} --json'.formatArgs(virtualNetworkPeerings);
        networkUtil.createGroup(groupName, location, suite, function () {
          testUtils.executeCommand(suite, retry, cmd, function (result) {
            result.exitStatus.should.equal(0);
            var output = JSON.parse(result.text);
      ...
      it('list should display all vnet peerings in resource group', function (done) {
      ...
      it('delete should delete vnet peering', function (done) {
