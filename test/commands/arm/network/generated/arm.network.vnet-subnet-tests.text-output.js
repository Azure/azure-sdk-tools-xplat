  ...
  describe('network', function () {
    ...
    describe('subnet', function () {
      ...
      it('create should create subnet', function (done) {
        var cmd = 'network vnet subnet create -g {group} -n {name} --address-prefix {addressPrefix} --private-access-services {privateAccessServices} --vnet-name {virtualNetworkName} --network-security-group-name {networkSecurityGroupName} --route-table-name {routeTableName}'.formatArgs(subnets);
        testUtils.executeCommand(suite, retry, cmd, function (result) {
          result.exitStatus.should.equal(0);
          done();
        });
      ...
      it('set should update subnet', function (done) {
        var cmd = 'network vnet subnet set -g {group} -n {name} --address-prefix {addressPrefixNew} --private-access-services {privateAccessServicesNew} --vnet-name {virtualNetworkName}'.formatArgs(subnets);
        testUtils.executeCommand(suite, retry, cmd, function (result) {
          result.exitStatus.should.equal(0);
          done();
        });
