  ...
  describe('network', function () {
    ...
    describe('url path maps', function () {
      ...
      it('create should create url path maps', function (done) {
        var cmd = 'network application-gateway url-path-map create -g {group} -n {name} --path {paths} --rule-name {pathRuleName} --gateway-name {applicationGatewayName} --address-pool-name {backendAddressPoolName} --http-settings-name {backendHttpSettingsCollectionName}'.formatArgs(urlPathMaps);
        testUtils.executeCommand(suite, retry, cmd, function (result) {
          result.exitStatus.should.equal(0);
          done();
        });
      ...
      it('show should display url path maps details', function (done) {
        var cmd = 'network application-gateway url-path-map show -g {group} -n {name} --gateway-name {applicationGatewayName}'.formatArgs(urlPathMaps);
        testUtils.executeCommand(suite, retry, cmd, function (result) {
          result.exitStatus.should.equal(0);
          done();
        });
      ...
      it('list should display all url path maps in resource group', function (done) {
        var cmd = 'network application-gateway url-path-map list -g {group} --gateway-name {applicationGatewayName}'.formatArgs(urlPathMaps);
        testUtils.executeCommand(suite, retry, cmd, function (result) {
          result.exitStatus.should.equal(0);
          done();
        });
      ...
      it('delete should delete url path maps', function (done) {
        var cmd = 'network application-gateway url-path-map delete -g {group} -n {name} --gateway-name {applicationGatewayName} --quiet'.formatArgs(urlPathMaps);
        testUtils.executeCommand(suite, retry, cmd, function (result) {
          result.exitStatus.should.equal(0);

          cmd = 'network application-gateway url-path-map show -g {group} -n {name} --gateway-name {applicationGatewayName}'.formatArgs(urlPathMaps);
          testUtils.executeCommand(suite, retry, cmd, function (result) {
            result.exitStatus.should.equal(0);
            done();
          });
