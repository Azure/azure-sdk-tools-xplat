...
var groupName, location,
  groupPrefix = 'xplatTestGExpressRouteAuth';
...
describe('arm', function () {
  describe('network', function () {
    ...
    describe('express-route authorization', function () {
      it('create should pass', function (done) {
        networkUtil.createGroup(groupName, location, suite, function () {
          networkUtil.createExpressRoute(groupName, expressRCPrefix, location, serviceProvider, peeringLocation, skuTier, skuFamily, tags1, suite, function () {
            var cmd = util.format('network express-route authorization create %s %s %s -k %s --json', groupName, expressRCPrefix, expressRCAuthPrefix, key1).split(' ');
      ...
      it('list should dispaly all express-routes authorization from resource group', function (done) {
      ...
      it('delete should delete express-route authorization', function (done) {
        var cmd = util.format('network express-route authorization delete %s %s %s --quiet --json', groupName, expressRCPrefix, expressRCAuthPrefix).split(' ');
        testUtils.executeCommand(suite, retry, cmd, function (result) {
          result.exitStatus.should.equal(0);
          done();
        });
